...
namespace cycfi::q {
    ...
    public:
        ...
        pd_preprocessor(
                Config const &conf, frequency lowest_freq, frequency highest_freq, std::uint32_t sps
        );
    ...
    inline pd_preprocessor::pd_preprocessor(
            Config const &conf, q::frequency lowest_freq, q::frequency highest_freq,
            std::uint32_t sps
    )
            : _env(conf.comp_release, sps), _comp(conf.comp_threshold, conf.comp_slope),
              _gate(float(conf.gate_off_threshold), float(conf.gate_on_threshold)),
              _lp1(highest_freq * 2, sps), _lp2(lowest_freq / 2, sps),
