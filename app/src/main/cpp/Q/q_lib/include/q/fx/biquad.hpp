...
namespace cycfi::q {
    ...
    struct biquad {
        ...
        biquad(float a0, float a1, float a2, float a3, float a4)
                : a0(a0), a1(a1), a2(a2), a3(a3), a4(a4), x1(0), x2(0), y1(0), y2(0) {}
    ...
    namespace detail {
        ...
        struct config_biquad {
            config_biquad(frequency f, std::uint32_t sps)
                    : omega(2_pi * double(f) / sps), sin(std::sin(omega)), cos(std::cos(omega)) {}
        ...
        struct config_biquad_a : config_biquad {
            config_biquad_a(double db_gain, frequency f, std::uint32_t sps, bw _bw)
                    : config_biquad(f, sps, _bw), a(std::pow(10.0, db_gain / 40.0)),
                      beta(std::sqrt(a + a)) {}

            config_biquad_a(double db_gain, frequency f, std::uint32_t sps, double q)
                    : config_biquad(f, sps, q), a(std::pow(10.0, db_gain / 40.0)),
                      beta(std::sqrt(a + a)) {}
