...
namespace cycfi::q {
    ...
    public:
        ...
        period_detector(
                frequency lowest_freq, frequency highest_freq, std::uint32_t sps, decibel hysteresis
        );
    ...
    inline period_detector::period_detector(
            ...
            : _zc(hysteresis, float(lowest_freq.period() * 2) * sps),
              ...
              _range(float(highest_freq) / float(lowest_freq)), _bits(_zc.window_size()),
              _weight(2.0 / _zc.window_size()), _mid_point(_zc.window_size() / 2),
    ...
    namespace detail {
        struct sub_collector {
            ...
            sub_collector(zero_crossing const &zc, float period_diff_threshold, int range_)
                    : _zc(zc), _harmonic_threshold(
                    period_detector::harmonic_periodicity_factor * 2 / zc.window_size()),
                      _period_diff_threshold(period_diff_threshold), _range(range_) {}
            ...
            bool
            try_sub_harmonic(std::size_t harmonic, info const &incoming, float incoming_period) {
            ...
            void get(info const &info, period_detector::info &result) {
                if (info._period != -1.0f) {
                    result =
                            {
                                    period_of(info) / info._harmonic, info._periodicity
                            };
                } else {
    ...
    inline int period_detector::autocorrelate(bitstream_acf<> const &ac, std::size_t &period,
                                              bool first) const {
