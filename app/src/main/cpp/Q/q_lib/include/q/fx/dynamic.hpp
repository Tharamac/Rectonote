...
namespace cycfi::q {
    ...
    struct compressor {
        constexpr compressor(decibel threshold, float ratio)
                : _threshold(threshold), _slope(1.0f - ratio) {}
    ...
    struct soft_knee_compressor {
        constexpr soft_knee_compressor(decibel threshold, decibel width, float ratio)
                : _threshold(threshold), _width(width), _lower(threshold - (_width * 0.5)),
                  _upper(threshold + (_width * 0.5)), _slope(1.0f - ratio) {}
    ...
    struct expander {
        constexpr expander(decibel threshold, float ratio)
                : _threshold(threshold), _slope(ratio) {}
