...
namespace cycfi::q {
    ...
    public:
        ...
        wav_writer(
                std::string filename, std::uint32_t num_channels, std::uint32_t sps)
                : wav_writer(filename.c_str(), num_channels, sps) {}

        wav_writer(
                char const *filename, std::uint32_t num_channels, std::uint32_t sps);
    ...
    inline wav_memory::wav_memory(char const *filename, std::size_t buff_size)
            : wav_reader(filename), _buff(*this ? buff_size * num_channels() : num_channels()) {
        if (*this)
