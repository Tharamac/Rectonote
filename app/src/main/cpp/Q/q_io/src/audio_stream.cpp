...
namespace cycfi::q {
    namespace detail {
        ...
        int audio_stream_callback1(
                void const *input_, void *output_, unsigned long frame_count,
                PaStreamCallbackTimeInfo const *time_info, PaStreamCallbackFlags status_flags,
                void *user_data
        ) {
    ...
    port_audio_stream::port_audio_stream(
            audio_device const &device, std::size_t input_channels, std::size_t output_channels,
            int sps, int frames
    ) {
        ...
        auto callback = (input_channels && output_channels) ?
                        ...
                        (input_channels ? detail::audio_stream_callback2
                                        : detail::audio_stream_callback3);

        auto err = Pa_OpenStream(
                reinterpret_cast<void **>(&_impl), input_channels ? &in_params : nullptr,
                output_channels ? &out_params : nullptr, sps, frames, paNoFlag, callback, this
        );
        if (err != paNoError) {
    ...
    port_audio_stream::port_audio_stream(
            std::size_t input_channels, std::size_t output_channels, int sps, int frames
    ) {
        // Make sure we're initialized
        detail::portaudio_init();
        ...
        auto err = Pa_OpenDefaultStream(
                reinterpret_cast<void **>(&_impl), input_channels, output_channels,
                paFloat32 | paNonInterleaved, sps, frames, callback, this
        );
        if (err != paNoError) {
            _error = Pa_GetErrorText(err);
            _impl = nullptr;
