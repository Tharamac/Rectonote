...
void printSupportedStandardSampleRates(
    ...
    for (int i = 0; STANDARD_SAMPLE_RATES[i] > 0; ++i) {
        portaudio::StreamParameters tmp = portaudio::StreamParameters(inputParameters,
                                                                      outputParameters,
                                                                      STANDARD_SAMPLE_RATES[i], 0,
                                                                      paNoFlag);
...
int main(int, char *[]) {
    try {
        ...
        for (portaudio::System::DeviceIterator i = sys.devicesBegin(); i != sys.devicesEnd(); ++i) {
            ...
            std::cout << "Max inputs = " << (*i).maxInputChannels() << ", Max outputs = "
                      << (*i).maxOutputChannels() << std::endl;

            std::cout << "Default low input latency   = " << (*i).defaultLowInputLatency()
                      << std::endl; // 8.3
            std::cout << "Default low output latency  = " << (*i).defaultLowOutputLatency()
                      << std::endl; // 8.3
            std::cout << "Default high input latency  = " << (*i).defaultHighInputLatency()
                      << std::endl; // 8.3
            std::cout << "Default high output latency = " << (*i).defaultHighOutputLatency()
                      << std::endl; // 8.3
            ...
            std::cout << "Default sample rate         = " << (*i).defaultSampleRate()
                      << std::endl; // 8.2

            // Poll for standard sample rates:
            portaudio::DirectionSpecificStreamParameters inputParameters((*i),
                                                                         (*i).maxInputChannels(),
                                                                         portaudio::INT16, true,
                                                                         0.0, NULL);
            ...
            if (inputParameters.numChannels() > 0) {
                ...
                std::cout << " for half-duplex 16 bit " << inputParameters.numChannels()
            ...
            if (outputParameters.numChannels() > 0) {
                std::cout << "Supported standard sample rates" << std::endl;
                std::cout << " for half-duplex 16 bit " << outputParameters.numChannels()
                          << " channel output = " << std::endl;
                printSupportedStandardSampleRates(
                        portaudio::DirectionSpecificStreamParameters::null(), outputParameters);
            }

            if (inputParameters.numChannels() > 0 && outputParameters.numChannels() > 0) {
                std::cout << "Supported standard sample rates" << std::endl;
                std::cout << " for full-duplex 16 bit " << inputParameters.numChannels()
                          << " channel input, " << outputParameters.numChannels()
                          << " channel output = " << std::endl;
