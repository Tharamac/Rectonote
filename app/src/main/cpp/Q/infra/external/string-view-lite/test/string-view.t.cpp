...
// 24.4.2.7 Searching:
    ...
    CASE("string_view: Allows to search backwards for the last occurrence of any of the characters specified in a string view, starting at position pos (default: npos) via find_last_of(), (1)") {
        ...
        string_view sve(empty);
                ...
                EXPECT(sve.find_last_of(string_view("x")) ==
    ...
    CASE("string_view: Allows to search backwards for the first character not specified in a string view, starting at position pos (default: npos) via find_last_not_of(), (1)") {
        ...
        string_view sve(empty);
                ...
                EXPECT(sve.find_last_not_of(string_view("x")) ==
                       string_view::npos);    // issue 20 (endless loop)
    }
...
#else
                EXPECT(
                !!"Literal operator 'sv' for string_view not available (nssv_CONFIG_STD_SV_OPERATOR=0).");
#endif
    }
...
#else
                EXPECT(
                !!"Literal operator '_sv' for string_view not available (nssv_CONFIG_USR_SV_OPERATOR=0).");
#endif
    }
...
#else
                EXPECT(
                !!"Conversion to/from std::string is not available (nssv_CONFIG_CONVERSION_STD_STRING_CLASS_METHODS=0).");
#endif
    }
...
#else
                EXPECT(
                !!"Conversion to/from std::string is not available (nssv_CONFIG_CONVERSION_STD_STRING_FREE_FUNCTIONS=0).");
#endif
    }
