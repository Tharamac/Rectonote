...
os.
replace( output_path, input_path
)
else:
os.
remove( input_path )
...
if verbose:
print( "- {path} => '{text}':".
format( path = in_path, text = new_text.strip('\n')
))
...
:
if verbose:
print( "Editing files to version {v}:".
format(v = version
))
for
item in
...
editFilesToVersion( args
.version[0], table, args.verbose )
