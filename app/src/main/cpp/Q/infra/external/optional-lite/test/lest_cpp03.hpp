    ...
    public:
        explicit approx(double magnitude)
                : epsilon_(100.0 * static_cast<double>( std::numeric_limits<float>::epsilon())),
                  scale_(1.0), magnitude_(magnitude) {}
        ...
        friend bool operator==(double lhs, approx const &rhs) {
            ...
            return lest::abs(lhs - rhs.magnitude_) < rhs.epsilon_ * (rhs.scale_ +
                                                                     (lest::min) (lest::abs(
                                                                             lhs), lest::abs(
                                                                             rhs.magnitude_)));
        }
        ...
        friend bool operator<=(double lhs, approx const &rhs) {
            return lhs < rhs.magnitude_ || lhs == rhs;
        }

        friend bool operator<=(approx const &lhs, double rhs) {
            return lhs.magnitude_ < rhs || lhs == rhs;
        }
...
// Expression decomposition:
    ...
    inline std::string to_string(char const &chr) {
        return "'" + make_tran_string(std::string(1, chr)) + "'";
    }

    inline std::string to_string(signed char const &chr) {
    ...
    inline std::string make_string(T const *ptr) {
        ...
        std::ostringstream os;
        os << std::internal << std::hex << std::showbase << std::setw(2 + 2 * sizeof(T *))
           << std::setfill('0') << reinterpret_cast<std::ptrdiff_t>( ptr );
        return os.str();
    }
    ...
    inline std::string make_string(R C::* ptr) {
        std::ostringstream os;
        os << std::internal << std::hex << std::showbase << std::setw(2 + 2 * sizeof(R C::*))
           << std::setfill('0') << ptr;
        return os.str();
    }
    ...
    template<typename T1, typename T2>
    std::string to_string(std::pair < T1, T2 >
    const & pair ) {
...
inline void report(std::ostream &os, message const &e, text test) {
    os << e.where << ": " << colourise(e.kind) << e.note << ": " << test << ": "
       << colourise(e.what()) << std::endl;
}
...
struct options {
    options()
            : help(false), abort(false), count(false), list(false), tags(false), time(false),
              pass(false), zen(false), lexical(false), random(false), verbose(false),
              version(false), repeat(1), seed(0) {}
...
struct confirm : action {
    ...
    ~confirm() {
        if (failures > 0) {
            os << failures << " out of " << selected << " selected " << pluralise("test", selected)
               << " " << colourise("failed.\n");
        } else if (output.pass()) {
            os << "All " << selected << " selected " << pluralise("test", selected) << " "
               << colourise("passed.\n");
        }
    }
};
...
struct rng {
    int operator()(int n) { return lest::rand() % n; }
};
...
inline seed_t seed(text opt, text arg) {
    ...
    throw std::runtime_error(
            "expecting 'time' or positive number with option '" + opt + "', got '" + arg +
            "' (try option --help)");
}
...
inline int repeat(text opt, text arg) {
    ...
    throw std::runtime_error(
            "expecting '-1' or positive number with option '" + opt + "', got '" + arg +
            "' (try option --help)");
}
...
inline int run(tests specification, texts arguments, std::ostream &os = std::cout) {
    try {
        ...
        if (option.count) {
            count count_(os);
            return for_test(specification, in, count_);
        }
        if (option.list) {
            print print_(os);
            return for_test(specification, in, print_);
        }
        if (option.tags) {
            ptags ptags_(os);
            return for_test(specification, in, ptags_);
        }
        if (option.time) {
            times times_(os, option);
            return for_test(specification, in, times_);
        }

        {
            confirm confirm_(os, option);
            return for_test(specification, in, confirm_, option.repeat);
        }
    }
    catch (std::exception const &e) {
        os << "Error: " << e.what() << "\n";
...
template<typename C>
test const *test_begin(C const &c) { return &*c; }

template<typename C>
test const *test_end(C const &c) { return test_begin(c) + lest_DIMENSION_OF(c); }

template<typename C>
char const *const *text_begin(C const &c) { return &*c; }

template<typename C>
char const *const *text_end(C const &c) { return text_begin(c) + lest_DIMENSION_OF(c); }
