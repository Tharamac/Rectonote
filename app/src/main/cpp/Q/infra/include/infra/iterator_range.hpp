...
namespace cycfi {
    ...
    public:
        ...
        static_assert(
                std::is_same<
                        typename traits::iterator_category, std::random_access_iterator_tag
                >::value, "Random access iterator required"
        );
    ...
    template<typename Iterator>
    template<std::size_t N>
    iterator_range<Iterator>::iterator_range(value_type(&arr)
    [N])
    :
